/*
 * camera.c - OV3660 camera initialization and capture
 */

#include "camera.h"
#include "config.h"
#include "esp_log.h"
#include "esp_camera.h"

/* Tag for logging */
static const char *TAG = "CAMERA";

/* Store the current frame buffer so we can release it later */
static camera_fb_t *s_fb = NULL;

bool camera_init(void)
{
    /* Camera configuration uses pins from config.h */
    camera_config_t config = {
        /* Data pins */
        .pin_d0 = CAM_PIN_D0,
        .pin_d1 = CAM_PIN_D1,
        .pin_d2 = CAM_PIN_D2,
        .pin_d3 = CAM_PIN_D3,
        .pin_d4 = CAM_PIN_D4,
        .pin_d5 = CAM_PIN_D5,
        .pin_d6 = CAM_PIN_D6,
        .pin_d7 = CAM_PIN_D7,

        /* Control pins */
        .pin_xclk = CAM_PIN_XCLK,
        .pin_vsync = CAM_PIN_VSYNC,
        .pin_href = CAM_PIN_HREF,
        .pin_pclk = CAM_PIN_PCLK,

        /* I2C pins (SCCB is the camera's I2C protocol) */
        .pin_sccb_sda = CAM_PIN_SDA,
        .pin_sccb_scl = CAM_PIN_SCL,

        /* Power and reset pins */
        .pin_pwdn = CAM_PIN_PWDN,
        .pin_reset = CAM_PIN_RESET,

        /* Clock frequency */
        .xclk_freq_hz = CAM_XCLK_FREQ,

        /* Frame buffer settings */
        .fb_count = 1,  /* Single frame buffer (not streaming) */
        .fb_location = CAMERA_FB_IN_PSRAM,  /* Frame buffer in PSRAM */
        .grab_mode = CAMERA_GRAB_WHEN_EMPTY,

        /* Image format */
        .pixel_format = PIXFORMAT_JPEG,
        .frame_size = FRAMESIZE_HD,  /* 1280x720 */
        .jpeg_quality = CAM_JPEG_QUALITY,  /* From config.h */
    };

    /* Initialize the camera */
    esp_err_t err = esp_camera_init(&config);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Camera initialization failed: %s", esp_err_to_name(err));
        return false;
    }

    ESP_LOGI(TAG, "Camera initialized (HD 1280x720, JPEG quality %d)", CAM_JPEG_QUALITY);
    return true;
}

bool camera_capture(uint8_t **out_buffer, size_t *out_length)
{
    /* Get a frame from the camera */
    s_fb = esp_camera_fb_get();
    
    if (s_fb == NULL) {
        ESP_LOGE(TAG, "Camera capture failed");
        *out_buffer = NULL;
        *out_length = 0;
        return false;
    }

    /* Return the buffer pointer and size to the caller */
    *out_buffer = s_fb->buf;
    *out_length = s_fb->len;

    ESP_LOGI(TAG, "Captured image: %zu bytes", s_fb->len);
    return true;
}

void camera_release_buffer(void)
{
    /* Return the frame buffer to the camera driver */
    if (s_fb != NULL) {
        esp_camera_fb_return(s_fb);
        s_fb = NULL;
        ESP_LOGI(TAG, "Frame buffer released");
    }
}

void camera_deinit(void)
{
    /* Release any held buffer first */
    camera_release_buffer();
    
    /* Deinitialize the camera */
    esp_err_t err = esp_camera_deinit();
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "Camera deinit warning: %s", esp_err_to_name(err));
    } else {
        ESP_LOGI(TAG, "Camera deinitialized");
    }
}
